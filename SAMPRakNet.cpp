...
uint8_t* SAMPRakNet::Decrypt(uint8_t const* src, int len) {
    ...
    static uint8_t sampDecrTable[256];
    static bool tableInitialized = false;
    
    if (!tableInitialized) {
        for (int i = 0; i < 256; i++) {
            sampDecrTable[sampEncrTable[i]] = i;
        }
    ...
    uint8_t port_xor = GetPort() ^ 0xAA;
    
    // === Упрощенный алгоритм - только kyretardizeDatagram ===
    // Если нужно обрабатывать EncryptOutcomingData, это будет отдельно
    
    bool apply_xor = false;
    uint8_t checksum = 0;
    
    // ДЕБАГ: Вывод входящих данных
    #ifdef _DEBUG
    printf("[DECRYPT] Incoming packet, len=%d, first byte=0x%02X\n", len, src[0]);
    #endif
    
    // Пробуем два варианта: apply_xor = false и apply_xor = true
    
    // Вариант 1: apply_xor = false (начинаем без XOR)
    for (int i = 1; i < len; ++i) {
        uint8_t cur = src[i];
        
        // Порядок ДЛЯ ДЕШИФРАЦИИ: сначала таблица, потом XOR (обратный шифрованию)
        cur = sampDecrTable[cur];
    ...
    if (src[0] == checksum) {
        #ifdef _DEBUG
        printf("[DECRYPT] Success with apply_xor=false, checksum=0x%02X\n", checksum);
        #endif
        return decryptBuffer_;
    }
    
    // Вариант 2: apply_xor = true (начинаем с XOR)
    apply_xor = true;
    checksum = 0;
    ...
    for (int i = 1; i < len; ++i) {
        ...
        // Другой возможный порядок: сначала XOR, потом таблица
        if (apply_xor) {
            cur ^= port_xor;
        }
    ...
    if (src[0] == checksum) {
        #ifdef _DEBUG
        printf("[DECRYPT] Success with apply_xor=true, checksum=0x%02X\n", checksum);
        #endif
        return decryptBuffer_;
    }
    
    // Вариант 3: альтернативный алгоритм (для двойного шифрования)
    if (src[0] == 27 && len > 10) {
        #ifdef _DEBUG
        printf("[DECRYPT] Trying double encryption detection (byte 27)\n");
        #endif
        
        // Простой вариант: игнорируем первый байт (27) и пробуем стандартную дешифрацию
        for (int i = 2; i < len; ++i) {  // Начинаем с индекса 2 (пропускаем 27 и следующий байт)
            uint8_t cur = src[i];
            
            // Пробуем оба варианта
            bool try_xor = ((i - 2) % 2 == 0); // Чередуем
            ...
            if (i - 2 < MAXIMUM_MTU_SIZE - 1) {
                decryptBuffer_[i - 2] = cur;
            }
        }
        
        #ifdef _DEBUG
        printf("[DECRYPT] Double encryption fallback applied\n");
        #endif
        return decryptBuffer_;
    }
    
    #ifdef _DEBUG
    printf("[DECRYPT] FAILED - no valid decryption found\n");
    #endif
