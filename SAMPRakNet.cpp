    ...
    Decrypt(uint8_t const* src, int len)
{
    ...
    // КРИТИЧЕСКИ ВАЖНО: определить правильное начальное значение!
    // В оригинале: do { unk ^= 1u; } while (v4);
    // unk начинает с 0 или 1? Проверим логику:
    // - v20 = unk == 0? Тогда если v20 == 0, применяем XOR
    // - Значит при unk = 0, XOR НЕ применяется? Нужно тестировать!
    
    bool apply_xor = false;  // Начинаем с false (XOR не применяется)
    uint8_t checksum = 0;
    ...
    for (int i = 1; i < len; ++i) {
        ...
        // ВАЖНО: Порядок ОБРАТНЫЙ шифратору!
        // Шифратор: таблица -> XOR
        // Дешифратор: XOR -> обратная таблица (или наоборот?)
        
        // Давайте проанализируем:
        // Шифрование: E(x) = XOR(Table[x]) или Table[XOR(x)]?
        // Из кода: encrBuffer[i] = Table[data[i]] ^ (condition ? port_xor : 0)
        
        // Значит расшифровка должна быть:
        // Если был XOR при шифровании, нужно сначала XOR обратно
        
        // Правильный алгоритм:
        // 1. Если при шифровании применялся XOR, применяем его обратно
        if (apply_xor) {
            cur ^= port_xor;  // XOR обратно
        }
        
        // 2. Обратная таблица
        uint8_t decrypted = sampDecrTable[cur];
        
        // 3. Контрольная сумма
        checksum ^= decrypted & 0x55;
        
        decryptBuffer_[i - 1] = decrypted;
        
        // Переключаем флаг для следующего байта
        apply_xor = !apply_xor;
    }
